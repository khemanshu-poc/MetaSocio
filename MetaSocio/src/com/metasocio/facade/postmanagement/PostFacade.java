...
public class PostFacade 
{
	...
	public List<Post> retrievePostWithImageOnHome(Session session, int startIndex, int maximumResult, int groupId) throws MetaSocioException 
	{
		...
		postsWithImage = iPostDao.retrievePostWithImageOnHome(session,startIndex,maximumResult,groupId);
			...
			Session session) throws MetaSocioException
	{
		...
		PostDao iPostDao = PostDao.getInstance();
		//dao is called
		return iPostDao.incrementLikesOnPost(postId,session);
	}
	/**
	...
	public int decrementLikesOnPost(int postId,Session session)  throws MetaSocioException
	{
	   ...
	   return iPostDao.decrementLikesOnPost(postId,session);
	}

	public int deletePostByPostId(int postId, Session session) throws MetaSocioException {
		PostDao iPostDao = PostDao.getInstance();
		// decrement likes on post
	    int check =  iPostDao.deletePostByPostId(postId,session);
	...
	public int editPostByPostId(int postId, String postDetails, Session session) throws MetaSocioException {
		PostDao iPostDao = PostDao.getInstance();
		// decrement likes on post
	   return iPostDao.editPostByPostId(postId,postDetails,session);
	}
}
