 ...
 * Description : This class is directly querying the database regarding groups to get the required information.
 */
public class GroupDao {
	 ...
	 * @param group : Containing all info, required to create new group
	...
	public int createGroup(Group group, Session session)
			throws MetaSocioException {
		 	...
		 	String hql = "Select g.groupId From Group g WHERE g.groupId =(Select max(groupId)from Group))";		//query to get Group id of newly created group
		 	Query query = session.createQuery(hql);				//creating query
		 	int groupId = (int) query.uniqueResult();			//query executed and returning unique result(group id)
		 	return groupId;										// Returning group id of newly created group 
	}

	
	/**
	 * This function is to update group information like :
	...
	public void updateGroup(Group group, Session session) throws MetaSocioException {
		//group.getUsersSet().remove(member);
		session.saveOrUpdate(group);		//Querying database to update group information
	}


	/**
	 * This function is to get complete information of a particular group on the basis of group id which is unique
	 * @param groupId : group id of group whose information is required
	 * @param session : Creating a session to process 
	 *					query.
	 ...
	 * 								if any exception occurs
	 */
	public Group getGroupById(int groupId, Session session) throws MetaSocioException {
	...
	/**
	 * This function is to get list of groups in which current user is not member
	...
	public List<Group> getGroupsInWhichUserIsNotMember(User user, List<Group> groupList,
		...
		List<Group> groupListInWhichUserIsNotMember = new ArrayList<Group>();	//Creating an instance of list which will contain group list in which user is not member

		List<Integer> groupIdSetOfMember = new ArrayList<Integer>();		/*Creating an instance of list which will 
									contain group id of those groups in which user is member*/
		
		//adding group ids of those groups to list in which user is member
		for(Group g : user.getGroupsSet()){		
			groupIdSetOfMember.add(g.getGroupId());
		}		
		//Adding groups to list in which user is not member
		for(Group g : groupList){
			if(!(groupIdSetOfMember.contains(g.getGroupId()))){
				groupListInWhichUserIsNotMember.add(g);
			}
		}
	...
	/**
	 * This function is to get list of users who are not the members of this group whose group id is provided
	 ...
	 * 								if any exception occurs
	 */
	public List<User> getUsersHavingSameDepartmentWhoAreNotGroupMembers(int groupId,
			List<User> users, Session session) throws MetaSocioException {
			...
			userSet = group.getUsersSet();								//setting all group members into usersSet
				...
				for(User user : users){								
					...
					usersWhoAreNotGroupMembers = users;				//if group does not contain any member, setting complete user list to list
			}
	 ...
	 * 								if any exception occurs
	 */
	public List<Group> getMyGroups(int userId, Session session) throws MetaSocioException {
